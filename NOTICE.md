...
0.8.4 增加的是“互操作适配”，不是复制第三方插件实现：心跳回忆只依据外部记忆服务/插件已公开使用的接口形状读取当前聊天窗口资料。当前内置兼容层不会导入第三方源码，也不会把第三方代码合并进本仓库。

本仓库原创代码采用根目录 `LICENSE` 所载 Tokimemo Proprietary Test License v1.0，非开源授权。
