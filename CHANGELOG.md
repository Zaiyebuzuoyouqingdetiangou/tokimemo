# 0.99.44 / r84.96 — 一份回忆最多两次，自动留忆会补上第二次

- 模块介绍里的次数改成这一份回忆实际要调用的次数：一次就完整的是 1 次，需要补全的是 2 次。两次合在一起才是一份完整回忆。
- 打开自动留忆后，这些模块会自动做第二次生成，不用再手动点第二次。手动生成仍看连接设置里的开关。
- 自动留忆设置里可以填写失败后重试次数，范围 1 到 5。自动留忆没写完时会按这个次数再试。

# 0.99.43 / r84.95 — 先完成共通设置，再问要不要自动留忆

- 模块介绍里的次数是生成这一份回忆要调用 API 几次。
- 向导先走文字 API、人物、聊天读取范围、可跳过的生图，以及建档。已有档案仍跳过建档。
- 模块介绍下面可以直接勾选这次要不要先生成，不再单独一页。
- 共通步骤结束后才问要不要自动留忆。不要就结束，之后仍能手动生成，也能在设置里打开或关闭。要的话再勾选自动生成、填间隔，最后确认。

# 0.99.42 / r84.94 — 待重试可以接着做，已有档案不再建档

- 任务中心里标成「待重试」的当前聊天任务会带上「重试未完成部分」。聊天经历整理会接着未完成的分块；没有草稿时按当前条件另起，不覆盖已经在的档案。超出保存上限的草稿和没有所属的旧记录仍然不能重试。
- 向导发现当前聊天已经有档案时，跳过建档那一步，确认页也不会为建档发请求。人物改过时仍会先问要不要重建。
- 设置页第一项是自动留忆。

# 0.99.41 / r84.93 — 向导里直接接上 API，并看懂这次生成什么

- API 还没接上时，向导这一步可以直接读取酒馆当前连接，或填写地址、Key 和模型。不用先返回设置。
- 模块介绍和勾选合在同一步。每一项写明会生成什么、大约请求几次，左边有勾选框，上面可以全选。取消勾选的以后不会被抽中。
- 成就不单独选。每生成一份回忆，都会带上对应的成就。
