...
- 一键配置新增独立“使用流式输出”开关；关闭时保留原非流式行为。
- 流式结果由 Connection Manager 官方返回的 AsyncGenerator 完整收齐后，再进入原 JSON / 来源 / 档案校验。
- 流中断时保留未完成草稿，不把半截 JSON 当作成功分块，也不自动重发付费请求。
- 保留 r70：更换模型、Profile、手动 API、温度或最大输出后，可只继续未完成档案分块。

# 0.8.65 / r70.0

- 档案整理/简介续写不再将连接、模型、请求预算或采样设置作为草稿的源身份；仅本地档案 ticket 排除单段 transport 参数，其他模式恢复引擎保持原样。
- 已成功分块重用时重新经过原 normalizer；未完成/截断分块使用本次配置继续，保留草稿与旧档案。
- 请求在途配置变化仍拒收旧结果；聊天来源/版本/分块/读取校验变化仍阻断。确认提示改为使用当前模型补未完成部分。
- 生图、悬浮球、导航和存储代码不改；页面草稿不跨刷新迁移。

# 0.8.64 / r69.0

- D 款悬浮球：透明原图裁切、48px、纯静态，无旋转/呼吸/闪烁/自动吸附动画。
- 指针拖动与点击分开；拖动松手不误开，位置有界且只在结束时保存。
- 键盘、屏幕旋转/可视区域变化、开关/重置、停用清理；点击复用原导航入口。
- 不改任何生成提示词/生图载荷、档案结构、历史证据或删除规则。
...
- 完整记录档案室页面层级；角色卷宗、档案简介、历史只读档案与模块阅读状态统一恢复。
- 书签仅含有界身份/导航数据；重读当前有效内容，旧版本/删除/同模式写入栅栏变化不恢复旧内容。
- 关闭、切聊、停用与新导航取消迟到恢复；有界滚动恢复在用户操作时立即停止。
- 原图、生图调用、角色绑定、记忆扫描、隐藏心跳、存储及生成规则未改。

# 0.8.59 / r63.0 — 本地修复候选

- 原图改为页内查看，增加有界文件准备/分享；TT 不走外跳兜底，并统一严格校验 tauri://localhost 本地图片资源。
- 图片外貌选择使用柏宝绘公开角色库；两人分开、目标隔离、后端能力检测，保持旧图与并发/取消规则。公开提示词生成入口不存在，未使用私有替代品。
- 千千结当前聊天已保存摘要进入现有来源账本；未就绪不清旧来源，身份变化拒绝，不引入 CSE 私密资料。
- CSS/SVG 室内与物件插画、卷宗列表、简介处增量更新、问号说明、按钮居中、目标角色性别称谓和去敏生成诊断。
- 隐藏心跳未修改。真实 TT/iPhone 与第三方服务待实测；历史测试对照仍有32项原基线已有失败，未虚构全绿或独立复核结果。
