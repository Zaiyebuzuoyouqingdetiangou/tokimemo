...
- 验证范围及仍保留的模型上下文、结构与数据保护边界见 REVIEW-r84.81.md。
...
- 恢复可重现的构建脚本与原模块初始化顺序，并随代码提供本轮回归测试。验证范围见 REVIEW-r84.80.md。
...
- manifest 入口缓存参数与版本号同步。测试边界见 REPAIR-r84.71.md。
...
- 保留 r84.69 无 12MB 字节上限及随信小画修复。测试边界见 REPAIR-r84.70.md。
...
- 反复出现「重试未完成部分」的原因写在 `问题分析-为什么会反复出现重试未完成部分.md`，这一轮不改提示词和重试次数。
