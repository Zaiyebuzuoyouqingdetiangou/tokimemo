...
export const MAX_DYNAMIC_RELATIONS = 14;

export const PROFILE_FACT_ORDER = Object.freeze(['生日', '年龄 / 年级', '身高', '血型', '职业 / 学校', '社团 / 工作', '兴趣', '喜欢的东西', '不喜欢的东西']);

const PROFILE_FACT_LABELS = new Set(PROFILE_FACT_ORDER);

export const PROFILE_DISCOVERY_LABELS = new Set([...PROFILE_FACT_LABELS, '习惯', '擅长的事', '害怕的东西', '重要的人 / 事物']);

export const RELATION_LAYERS = new Set(['family', 'close', 'friend', 'work', 'school', 'rival', 'acquaintance', 'special']);

export const RELATION_STATES = new Set(['亲密', '友好', '普通', '疏远', '紧张', '敌对', '竞争', '复杂', '恋爱', '暧昧', '伴侣', '家人', '同事', '同学', '师生', '主从', '特殊']);
...
export function foldEvidence(value) {
    return core_text.normalizeText(value, 12000).replace(/\s+/g, '').toLocaleLowerCase();
}

export function relationParticipantNames(memoryBank, context = null) {
    const names = core_participants.resolveStoryIdentities(memoryBank, context).ownerNames;
    return [...new Set(names.map(name => core_text.normalizeText(name, 120)).filter(Boolean))];
}
...
export function factValueBackedByEvidence(value, evidence) {
...
export function normalizeProfileFactLabel(value) {
...
export function relationsPrompt(context, memoryBank, settingEntries = []) {
    ...
    const participantRule = participantNames.length
        ? `本次关系网的中心人物只可以是：${participantNames.join('、')}。角色卡名称只是资料容器，绝不能作为关系边的主人公。每条 relationships 必须写 ownerName，且只能是上述某一个真实人物。`
        : '每条 relationships 必须写 ownerName，且必须是当前档案中可核对的真实人物；角色卡名称不是自动的人物名。';
...
- 每条 relationships 的 ownerName 必须逐字等于本次选定名单中的真实人物名；所引用 Mxxx 的标题、摘要、anchor 或 participants 必须能同时核对 ownerName 与关系对象。不要把角色卡名称填进 ownerName。
