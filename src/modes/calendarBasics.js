...
export function pageMetaForKey(key) {
...
export function ensureCalendarDayPage(pages, key) {
    if (!key) return null;
...
export function folded(value, max = 180) {
...
export function ensureUniqueCalendarEntryIds(value) {
    const used = new Set();
...
export function ensureUniqueCalendarPageItems(value, prefix, { semanticKey = null, dedupeSemantic = false } = {}) {
...
export function normalizeCalendarPageCollections(page) {
...
export function resolveAnchoredMemory(memoryBank, sourceMemoryIds, sourceMemoryAnchor) {
...
export function resolveAnchoredDatedMemory(memoryBank, sourceMemoryIds, sourceMemoryAnchor) {
...
export function normalizePastMarkedEntries(value, memoryBank) {
...
export function normalizePromisedEntries(value, memoryBank) {
    const raw = Array.isArray(value) ? value : [];
    const out = [];
...
export function normalizeFutureEntries(value, { futureEvidenceText = '', holidayEvidenceText = '' } = {}) {
    const raw = Array.isArray(value) ? value : [];
    const out = [];
...
export function normalizeHolidayCards(value, entries, { currentDate = '', allowStored = false, memoryBank = null } = {}) {
    const raw = Array.isArray(value) ? value : [];
...
export function normalizeStickyNotes(value, memoryBank, { controlledEvidence = '' } = {}) {
    const raw = Array.isArray(value) ? value : [];
    const out = [];
