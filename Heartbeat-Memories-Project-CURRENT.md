...
- 产品名：心跳回忆
- version：`0.8.48`
- BUILD / runtime cache-bust：`0.8.48-evolution-r52.0`
...
V3 metadata key 为旧档案兼容边界，不随发布版本改名。浏览器加载 r52 runtime 由 manifest 与 index 中一致的 BUILD query 隔离，不能继续命中旧 bundle。

## r52 当前增补契约

- 初次 Butterfly 使用本地 MAIN / 八轴 / OMEGA 十槽位顺序请求；每槽独立有界修复，Ω 使用已校验前节点的有界内容，最后全量完整性校验才提交。成功槽在本轮内保留，不宣称刷新页面后仍可续跑。增量沿用较小的 1–3 分歧加 Ω 请求，不删除旧节点。
