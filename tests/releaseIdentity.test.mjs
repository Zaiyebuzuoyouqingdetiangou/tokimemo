...
test('public extension identity stays fixed while version is separate', async () => {
  ...
  assert.equal(manifest.version, '0.8.46');
  ...
  assert.match(readme, /当前版本：\*\*0\.8\.46（r50\.0）\*\*/);
  assert.match(changelog, /^# 0\.8\.46 \/ r50\.0/m);
  assert.match(architecture, /^# Heartbeat Memories r35–r50 Architecture/m);
  assert.match(architecture, /^## r49\.0 deep-review and ArchiveTarget contract/m);
  assert.match(security, /^## r49\.0 跨档案、连接与表现层安全边界/m);
  assert.match(securityReview, /^## 0\.8\.46 \/ r50\.0/m);
  assert.match(index, /const VERSION = '0\.8\.46'/);
  assert.match(index, /const BUILD = '0\.8\.46-feedback-r50\.0'/);
  assert.equal(manifest.js, 'index.js?heartbeat=0.8.46-feedback-r50.0');
