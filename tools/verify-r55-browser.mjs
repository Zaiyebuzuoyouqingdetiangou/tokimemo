...
const out = path.resolve(process.env.R55_BROWSER_OUTPUT || path.join(root, 'artifacts/r55-browser'));
...
const errors = [], cases = [];
const guard = setTimeout(() => { console.error('r55 browser verification exceeded 50 seconds'); process.exit(1); }, 50000);
    ...
    for (const [width, theme] of [[390, 'default'], [390, 'night'], [1280, 'default'], [390, 'host']]) {
        ...
        await context.route('**/*', route => {
            ...
            if (url.pathname === '/api/backends/chat-completions/generate') {
                requests++;
                return route.fulfill({ json: { choices: [{ finish_reason: 'stop', message: { content: JSON.stringify({
                    archiveName: '雨停之前', archiveVerdict: '把等待留在雨声里，把明天留给还没有说完的话。',
        ...
        assert.equal(await page.locator('.rmt-archive-verdict').textContent(), result.bank.archiveVerdict.text);
        assert.equal(await page.locator('.rmt-archive-source-fold').getAttribute('open'), null);
