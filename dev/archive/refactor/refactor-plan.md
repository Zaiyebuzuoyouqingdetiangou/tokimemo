...
### 阶段 3 · 理依赖 / 拆超大函数（2026-09-25 实测后细化，等用户选）

实测（r84.77，235 个模块）：

- 循环依赖：177 个模块在同一个循环里。把所有“经过转发层”的 import 改成直接指向真正定义的文件，循环一点都不会变小（仍是 177）——循环是各层互相调用造成的，不是拆分造成的。
- 向上依赖的 import 行：generation→modes 87、archive→ui 23、core→archive 21、generation→ui 19、modes→ui 17、core→generation 11、core→modes 11、archive→generation 9、archive→modes 5、core→ui 2。
- 剩下 3 个超过 60KB 的文件，大头都是单个函数：`handleOverlayClick` 44KB（111 个 `action === '…'` 分支）、`hydrateSettingsPanel` 58KB、`generateMode` 约 50KB。

可选方向（都要改写函数，refactor-guard 的“逐字相同”证明不再适用，只能靠测试和手点）：

- 3A 拆点击分发：把 `handleOverlayClick` 的 111 个分支按页面分成若干处理表，原函数只查表。先写“每个 action 都路由到同一段代码”的对照测试，再改。风险中等；影响全部按钮，必须整页手点。
- 3B 拆设置页填充：`hydrateSettingsPanel` 按设置分区拆成若干绑定函数。风险中等；影响全部设置项。
- 3C 解循环：generation→modes 改注册表、core / archive 向上调用改注入。涉及上百处调用和初始化顺序，风险最高、收益主要是以后改代码更安全，用户看不到变化。
- 3D 到此为止：阶段 2 的目标（单个会话读得完）已经达到，重构任务收尾归档。

建议：3D，或只做 3A（按钮最多、最常改）。3C 不建议现在做。
